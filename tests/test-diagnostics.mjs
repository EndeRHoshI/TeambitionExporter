...
globalThis.chrome={storage:{session:{async set(value){if(failStorage)throw Error('quota');Object.assign(store,value);},async get(key){if(failStorage)throw Error('unavailable');return key==null?store:{[key]:store[key]};}}},runtime:{getManifest(){return {version:'0.3.0'};}}};
